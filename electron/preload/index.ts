...
if (process.contextIsolated) {
	try {
		...
		contextBridge.exposeInMainWorld("electronAPI", {
			...
			shortcut: {
				onShortcutSync: (callback: (data: ShortcutSyncEvent) => void) => {
					ipcRenderer.on("shortcut:sync", (_, data) => callback(data));
				},
				onShortcutAction: (callback: (event: ShortcutActionEvent) => void) => {
					ipcRenderer.on("shortcut:action", (_, event) => callback(event));
				},
		...
		// Expose IPC listener for theme changes and broadcast events
