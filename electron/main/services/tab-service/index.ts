...
export class TabService {
	...
	private async resurrectTab(targetWindow: BrowserWindow, threadId: string): Promise<Tab | null> {
		...
		if (oldWindowId !== targetWindow.id) {
			...
			`,
				...
				.catch((err) => logger.error("Failed to update windowId during resurrection:", err));
	...
	async transferTabToWindow(
		...
		logger.info(`Transferring Tab ${tabId} from window ${fromWindow.id} to window ${toWindow.id}`);
	...
	async createTabForUrl(
		...
		if (!isNull(finalTabState)) {
			...
			finalTabState[windowId] = { tabs: updatedTabs };
			await tabStorage.setItemInternal("tab-bar-state", finalTabState);
			logger.info(`[TabService] Created tab ${newTabId} (${type}, ${href}) in window ${windowId}`);
		}
	...
	async handleNewTabWithThread(
		...
		logger.info(`[TabService] handleNewTabWithThread: threadId=${threadId}, windowId=${windowId}`);
		...
		if (!isNull(tabState)) {
			...
			for (const [wId, wData] of Object.entries(tabState)) {
				...
				if (existingTab) {
					...
					logger.info(`[TabService] View exists for tab ${existingTab.id}:`, !!existingView);
	...
	private isTabLimitReached(windowId: number): boolean {
		...
		if (views.length >= MAX_TABS_PER_WINDOW) {
			logger.warn(`[TabService] Tab limit (${MAX_TABS_PER_WINDOW}) reached for window ${windowId}`);
			return true;
		}
		return false;
	}
	...
	async handleClearTabMessages(
		...
		logger.info(`[handleClearTabMessages] Clearing messages for tab ${tabId}, thread ${threadId}`);
		...
		try {
			...
			if (isUndefined(view) || view.webContents.isDestroyed()) {
				logger.warn(`[handleClearTabMessages] View not found or destroyed for tab ${tabId}`);
	...
	async handleGenerateTabTitle(
		...
		logger.info(`[handleGenerateTabTitle] Generating title for tab ${tabId}, thread ${threadId}`);

		try {
			// Get the tab's WebContentsView
			const view = this.tabViewMap.get(tabId);
			if (isUndefined(view) || view.webContents.isDestroyed()) {
				logger.warn(`[handleGenerateTabTitle] View not found or destroyed for tab ${tabId}`);
				return false;
			}
