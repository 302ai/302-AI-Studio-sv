...
<script lang="ts">
	...
	$effect(() => {
		if (modelPanelState.isOpen && selectedModel) {
	...
	$effect(() => {
		if (!modelPanelState.isOpen) {
			scrollTop = 0;
		}
	});

	$effect(() => {
		if (modelPanelState.isOpen && listRef) {
			setTimeout(() => {
				if (!listRef) return;
...
<Command.Dialog bind:open={modelPanelState.isOpen} class="w-[638px]">
