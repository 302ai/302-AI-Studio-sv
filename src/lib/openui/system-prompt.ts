...
AspectRatio(ratio?: number, children?: string) — A container preserving a fixed aspect ratio. Props: ratio (number, e.g., 16/9), children (content to scale). Example: AspectRatio({ratio: 16/9}, Image({src: '...'}))
...
Badge(class?: string, variant?: "default" | "secondary" | "destructive" | "outline", href?: string, text?: string) — A small status badge. Props: variant ('default'|'secondary'|'destructive'|'outline'), text (string content), href (optional link). Use for labels, status indicators. Example: Badge({variant: 'secondary', text: 'New'})
...
Button(class?: string, variant?: "default" | "destructive" | "outline" | "secondary" | "ghost" | "link", size?: "default" | "sm" | "lg" | "icon" | "icon-sm" | "icon-lg", href?: string, type?: "button" | "submit" | "reset", disabled?: boolean, children?: string) — A clickable button. Props: variant ('default'|'destructive'|'outline'|'secondary'|'ghost'|'link'), size ('default'|'sm'|'lg'|'icon'), href (link URL), disabled, children (label). Example: Button({variant: 'default', size: 'lg', children: 'Click Me'})
Card(class?: string, title: string, description?: string, children?: string, footer?: string) — A card container. Props: title, description (optional), children (optional), footer (optional). Example: Card('Title', 'Description', content, 'Footer')
...
Label(class?: string, for?: string, children?: string) — A label for form controls. Props: for (associated input id), children (label text). Links to form inputs for accessibility. Example: Label({for: 'email', children: 'Email Address'})
...
ScrollArea(class?: string, orientation?: "vertical" | "horizontal" | "both", children?: string) — A scrollable content area. Props: orientation ('vertical'|'horizontal'|'both'), children (scrollable content). Example: ScrollArea({orientation: 'vertical', children: LongContent()})
...
Stack(children: string, direction: "vertical" | "horizontal", gap?: "s" | "m" | "l", class?: string) — A flexible stack layout. Props: children (array), direction ('vertical'|'horizontal'), gap ('s'|'m'|'l'). Example: Stack(children, 'horizontal', 'm')
...
Toggle(class?: string, pressed?: boolean, disabled?: boolean, variant?: "default" | "outline", size?: "default" | "sm" | "lg", children?: string) — A pressable toggle control (button-style). Props: variant ('default'|'outline'), size ('default'|'sm'|'lg'), pressed (boolean), disabled, children (label). Example: Toggle({variant: 'outline', size: 'default', children: 'Bold'})
