...
export function createLogger(category: LogCategory): Logger {
	if (isMainProcess()) {
		return {
			debug: (message: string, ...args: any[]) => mainLogFn?.("debug", category, message, ...args),
			info: (message: string, ...args: any[]) => mainLogFn?.("info", category, message, ...args),
			warn: (message: string, ...args: any[]) => mainLogFn?.("warn", category, message, ...args),
			error: (message: string, ...args: any[]) => mainLogFn?.("error", category, message, ...args),
			fatal: (message: string, ...args: any[]) => mainLogFn?.("fatal", category, message, ...args),
		};
	}
