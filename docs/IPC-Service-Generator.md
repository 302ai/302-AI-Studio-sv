...
```typescript
{
  serviceName: "userService",      // UserService → userService（保留完整类名）
...
- **类名转服务名**：`UserService` → `userService`（保留完整类名并转为驼峰）
...
export const autoGeneratedServices: AutoGeneratedIpcServices = {
	windowService: {
		maximize: () => ipcRenderer.invoke("app:windowService:maximize"),
		minimize: () => ipcRenderer.invoke("app:windowService:minimize"),
		close: () => ipcRenderer.invoke("app:windowService:close"),
	},
};
```
...
export function registerIpcHandlers() {
	...
	const windowServiceInstance = new WindowService();
	ipcMain.handle("app:windowService:maximize", (event) => windowServiceInstance.maximize(event));
	ipcMain.handle("app:windowService:minimize", (event) => windowServiceInstance.minimize(event));
	ipcMain.handle("app:windowService:close", (event) => windowServiceInstance.close(event));
}
...
export function removeIpcHandlers() {
	ipcMain.removeHandler("app:windowService:maximize");
	ipcMain.removeHandler("app:windowService:minimize");
	ipcMain.removeHandler("app:windowService:close");
}
```
