...
export class IpcStructureGenerator {
			...
			if (hasGenericParams) {
				method.genericParameters.forEach((genericParam) => {
					if (genericParam.defaultType) {
						const typesInDefault = this.extractTypesFromString(
							genericParam.defaultType,
						);
					...
					if (genericParam.constraint) {
						const typesInConstraint = this.extractTypesFromString(
	...
	public generatePreloadServicesModule(structure: GeneratedIpcStructure): string {
		const servicesInterface = structure.services
			.map((service) => {
				const methods = service.methods
					.map((method) => {
						const genericString = this.generateGenericParametersString(
		...
		const extensionInterface = structure.services
			.map(
				(service) =>
					`${service.serviceName}: AutoGeneratedIpcServices['${service.serviceName}'];`,
			)
			.join("\n");
			...
			.map((service) => {
				...
				const methods = service.methods
					.map((method) => {
						...
						const handlerParams =
							businessParams.length > 0 ? `event, ${paramNames}` : "event";
