...
<!-- This class won't be detected --><div class="bg-{color}-500">Bad</div>
```
...
<div class:bg-blue-500={type === "primary"} class:bg-red-500={type === "secondary"}>Good</div>
```
...
```svelte
<button class="rounded bg-blue-500 px-4 py-2 text-white hover:bg-blue-600"> Button </button>
```
...
<!-- Card.svelte -->
<script>
	let { elevated = false, bordered = true, padding = "md", children } = $props();
...
```svelte
<div class="w-[var(--sidebar-width)] h-[var(--header-height)]">Layout element</div>
```
...
<div style="width: {width}px; height: {height}px">Dynamic sizing</div>
```
...
<div class="bg-white text-gray-900 dark:bg-gray-900 dark:text-white">
	<h1 class="text-3xl font-bold dark:text-blue-400">Title</h1>
...
```svelte
<div class="bg-[var(--bg-primary)] text-[var(--text-primary)]">Theme-aware content</div>
```
...
```svelte
<p class="text-balance">This text will balance across lines</p>

<div class="scrollbar-hide overflow-y-auto">Content without scrollbar</div>
```
...
<div class="flex flex-col md:flex-row">
	<aside class="w-full md:w-64 md:flex-shrink-0">Sidebar</aside>
...
```svelte
<div class="@container">
	<div class="@md:grid @md:grid-cols-2 @lg:grid-cols-3">Container-responsive grid</div>
</div>
```
...
{#if visible}
	<div transition:fade class="rounded bg-blue-500 p-4 text-white">Fades in/out</div>
{/if}
```
