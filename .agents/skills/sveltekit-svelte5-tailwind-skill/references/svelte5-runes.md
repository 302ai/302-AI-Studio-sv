...
<p>Count: {count}</p><p>Doubled: {doubled}</p><p>{isEven ? "Even" : "Odd"}</p>
```
...
<p>Total: ${total.toFixed(2)}</p><p>Items: {itemCount}</p><p>Average: ${average.toFixed(2)}</p>
```
...
<script>
	...
	let sortedItems = $derived(data.items.sort((a, b) => a.name.localeCompare(b.name)));
</script>
```
...
<Counter bind:count /><p>Parent sees: {count}</p>
```
