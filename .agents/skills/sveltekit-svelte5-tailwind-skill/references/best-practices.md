...
<script>
	...
	let filteredPosts = $derived(
		initialPosts
			...
			.sort((a, b) => (sortBy === "date" ? b.date - a.date : a.title.localeCompare(b.title))),
	);
</script>
```
...
<form method="POST" use:handleSubmit>
	<input name="content" bind:value={optimisticData} disabled={submitting} class="input" />
...
<script lang="ts">
	...
	let { variant = "primary", size = "md", disabled = false, children } = $props();
...
<div class="card" class:border-blue-500={isActive} class:border-gray-200={!isActive}>
	Card content
</div>
...
<button onclick={() => (showAdvanced = !showAdvanced)}> Toggle Advanced </button>
