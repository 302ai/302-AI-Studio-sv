...
<div class={statusColors[status]}>Status indicator</div>
```
...
<script lang="ts">
	...
	let { src, alt, width, height, priority = false, className = "" }: ImageProps = $props();
...
<nav>
	<a href="/dashboard" onmouseenter={() => handleMouseEnter("/dashboard")} class="nav-link">
		Dashboard
	</a>
	<a href="/profile" onmouseenter={() => handleMouseEnter("/profile")} class="nav-link">
		Profile
	</a>
</nav>
```
...
<a href="/dashboard" data-sveltekit-preload-data="hover" class="nav-link"> Dashboard </a>

<!-- Or preload on viewport entry -->
<a href="/profile" data-sveltekit-preload-data="viewport" class="nav-link"> Profile </a>
```
...
<img src="/banner.jpg" alt="Banner" width="1200" height="400" class="w-full h-auto" />
```
